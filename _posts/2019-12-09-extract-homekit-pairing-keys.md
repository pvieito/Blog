...
While the Home app allows you to read and control your HomeKit-based devices on macOS, iPadOS and iOS, sometimes you want more control. You may want to bridge a sensor to another protocol, expose your home temperature on a public API, or simply export the historical data to a CSV file. Either way, the `HomeKit` framework is currently a private API on macOS, and it is not available on other platforms like Linux or Windows.
...
In particular, **HomeKit Python** supports reading and writing HomeKit characteristics on paired devices as well as generating additional pairings. The main problem is that HomeKit devices typically support only one main pairing controller. Thus, once a device is paired with the Apple Home app, it can only be controlled with the pairing keys managed by the `homed` system daemon, which are gated by the `HomeKit` framework.

Fortunately, the HomeKit pairing keys are stored in iCloud Keychain. Unfortunately (_but reasonably_), the system tries hard to hide these pairing keys. In particular, they neither appear in the Keychain app nor can they be read with the `Security` framework `SecItem*` family of APIs without private entitlements granting access to the keychain access group `com.apple.hap.pairing`.

### Process

First of all, we have to subvert the AMFI security model to be able to sign arbitrary executables with private entitlements. To do it, we have to disable **System Integrity Protection** and **AMFI**. By rebooting into **Recovery OS**, we can disable the protections using the Terminal app:
...
- Each accessory has its own **Paired HomeKit Accessory** keychain entry containing its pairing key. For example, the device `58:CA:96:CE:66:E9` has the LTP key `44d34407d583aee3b12b774a6eb15ee96c527fa83af1db66ac90f60494bbbc29`.
...
# Write the pairing credentials for each device:
{
  "EveExtend": {
...
# The HomeKit device should identify itself (for example, by blinking an LED).
$ python3 -m homekit.get_accessories -f ~/.homekit_python/pairing.json -a EveExtend
# Shows all the accessories exposed by the device.
